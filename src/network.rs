...
impl<'a> neli::FromBytesWithInput<'a> for IgnorePayload {
    ...
    fn from_bytes_with_input(
        ...
        _input: Self::Input,
    ) -> Result<Self, DeError> { Ok(IgnorePayload) }
}
...
fn read_nl_reply(netlink: &mut NlSocketHandle) -> Result<(), Error> {
    for message in netlink.iter::<'_, Nlmsg, IgnorePayload>(false /* wait until we receive a Done message */) {
...
pub fn setup_network() -> Result<(), Error> {
    ...
    let mut netlink = check!(NlSocketHandle::new(
        NlFamily::Route,
    ...
    let loopback = check!(netdevice::get_index(netlink.as_raw_fd(), "lo"), "error looking up lo: {}");
